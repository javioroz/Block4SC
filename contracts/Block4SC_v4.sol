...
contract Block4SC {
     ...
     * 03.06.2022 | J.Oroz | v3.0 created mappings for quant_byMat_inWH, materials_byID, matID
     *                     | created functions createStock(), deleteStock(), testCreateStock(), getQuantityOfMatInWH(), 
     * 08.06.2022 | J.Oroz | new function getMaterialsInWH() and getAllStocks()
     * pending to develop getAllWareHouses() and getAllContainerIDs()
     */
    ...
    function getMaterialsInWH(string memory _warehouse) public view returns (string memory){
        ...
        for (uint _i=1; _i<numOfMaterials; _i++){
            ...
            _quant = quant_byMat_inWH[_material][_warehouse];
            if (_quant != 0){
                //_matList[_j]=materials_byID[_i];
                _matList = string.concat(_matList,materials_byID[_i]);
                _matList = string.concat(_matList,": ");
                _matList = string.concat(_matList, string(_quant));
                _matList = string.concat(_matList,", ");
                //_j++;
            }
        }
